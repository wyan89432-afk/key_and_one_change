...
// 1-change rule: order does not matter. Exactly two digits are shared and
// the remaining digit moves by +/-1 on a circular wheel (0 <-> 9).
function isOneChange(a, b) {
  const x = normalize3(a), y = normalize3(b);
  if (!/^\d{3}$/.test(x) || !/^\d{3}$/.test(y)) return false;
  const countA = Array(10).fill(0), countB = Array(10).fill(0);
    ...
    for (let col = 0; col < cols; col++) {
      for (let row = 0; row < rows; row++) {
        ...
        if (isOneChange(input, value)) {
          matches.push({ linear: col * rows + row, row, col, value, input, index: inputIndex + 1, color: COLORS[inputIndex % COLORS.length] });
        }
      }
    }
  });
...
// Sequential mode: pasted inputs must appear in their pasted order at exactly
// p+1 cells apart. 0p = every next cell, 1p = one cell skipped, ... 7p.
// The scan is column-major, so a chain can continue from row 24 into the next
// column's row 1 (or the corresponding row after the p-gap).
...
function drawSequentialArrows() {
  ...
  marker.setAttribute('id', 'blueArrowHead'); marker.setAttribute('markerWidth', '8'); marker.setAttribute('markerHeight', '8'); marker.setAttribute('refX', '7'); marker.setAttribute('refY', '4'); marker.setAttribute('orient', 'auto');
  const head = document.createElementNS('http://www.w3.org/2000/svg', 'path');
  head.setAttribute('d', 'M0,0 L8,4 L0,8 Z'); head.setAttribute('fill', '#1683ff'); marker.appendChild(head); defs.appendChild(marker); svg.appendChild(defs);
  ...
  const addArrow = (a, b) => {
    const ca = point(a), cb = point(b); if (!ca || !cb) return;
    ...
    if (a.col === b.col) {
      ...
      // Cross-column connection is routed outside the cell faces, so numbers
    ...
    path.setAttribute('d', d); path.setAttribute('class', 'sequence-arrow'); path.setAttribute('marker-end', 'url(#blueArrowHead)'); svg.appendChild(path);
  };
  state.sequentialChains.forEach(chain => { for (let i = 0; i < chain.length - 1; i++) addArrow(chain[i], chain[i + 1]); });
  wrap.appendChild(svg);
}

function calculate() {
  if (!state.data.length) return;
  const p = parseP($('pattern').value), nums = getNumbers();
  $('inputStats').textContent = `${nums.length} number${nums.length === 1 ? '' : 's'}`;
  if (p === null) { $('resultStatus').textContent = 'Please enter 0p, 1p, 2p, 3p, 4p, 5p, 6p or 7p.'; return; }
  if (!nums.length) {
    $('resultStatus').textContent = 'Paste at least one 3-digit number.';
    state.sequentialChains = []; state.matches = []; renderFixedTable(); $('resultTable').innerHTML = ''; return;
  }
  ...
  matches.forEach(match => { const key = `${match.col}:${match.row}`; if (!highlights.has(key)) highlights.set(key, match); });
...
function renderResults(matches, total, p, allCount = matches.length) {
  if (state.sequentialOnly) {
    $('resultStatus').textContent = matches.length
      ? `${matches.length} sequential matches kept • ${allCount - matches.length} non-sequential matches hidden • ${p}p.`
  ...
  const wrap = $('resultTable'); wrap.innerHTML = ''; if (!matches.length) return;
  ...
  matches.forEach(m => {
    const tr = document.createElement('tr');
    [m.index, m.input, m.value, state.headers[m.col], m.row + 1, m.color].forEach((v, i) => { const td = document.createElement('td'); td.textContent = v; if (i === 5) td.className = `badge badge-${m.color}`; tr.appendChild(td); });
    body.appendChild(tr);
  });
...
function addRow() {
  ...
  state.rowCount = state.data.length;
  renderFixedTable(); updateMeta('Edited table'); calculate();
}
...
$('inputNumbers').addEventListener('input', () => { $('inputStats').textContent = `${getNumbers().length} numbers`; });
...
$('sequentialBtn').addEventListener('click', () => {
  ...
  const btn = $('sequentialBtn'); btn.classList.toggle('active', state.sequentialOnly); btn.textContent = state.sequentialOnly ? 'အစဉ်လိုက် ✓' : 'အစဉ်လိုက်'; calculate();
});
$('addRowBtn').addEventListener('click', addRow);
$('addColumnBtn').addEventListener('click', addColumn);
$('clearBtn').addEventListener('click', () => {
  $('inputNumbers').value = ''; $('pattern').value = '0p'; $('resultTable').innerHTML = ''; $('inputStats').textContent = '0 numbers'; state.sequentialChains = [];
  if (state.sequentialOnly) { state.sequentialOnly = false; $('sequentialBtn').classList.remove('active'); $('sequentialBtn').textContent = 'အစဉ်လိုက်'; }
  if (state.data.length) renderFixedTable(); $('resultStatus').textContent = state.data.length ? 'Cleared.' : 'Upload the fixed table to begin.';
});
$('fileInput').addEventListener('change', async e => {
  const file = e.target.files?.[0]; if (!file) return;
  try { const wb = XLSX.read(await file.arrayBuffer(), { type: 'array' }); const ws = wb.Sheets[wb.SheetNames[0]]; loadMatrix(XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' }), file.name); }
  catch (err) { $('resultStatus').textContent = `Could not read file: ${err.message}`; }
});
async function loadDefault() {
  try { const res = await fetch('fixed-table.csv', { cache: 'no-store' }); if (!res.ok) throw new Error(); const wb = XLSX.read(await res.text(), { type: 'string' }); const ws = wb.Sheets[wb.SheetNames[0]]; loadMatrix(XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' }), 'fixed-table.csv'); }
  catch { $('resultStatus').textContent = 'Upload the fixed table to begin.'; }
}
loadDefault();
